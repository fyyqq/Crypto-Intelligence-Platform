...
4. Consider applying the same "never guess a bare exchange symbol" pattern to any other place in this codebase that might construct a TradingView/exchange symbol from raw ticker data.
5. If a coin ever turns out to have a real listing TradingView's search API *also* can't find (extremely obscure/new tokens), the chart will correctly show "No live chart available" rather than guessing — this is by design, not a gap to fix.

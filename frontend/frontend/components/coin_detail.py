...
def _highlighted_paragraph(tokens: rx.Var, color: str | None = None) -> rx.Component:
    return rx.text(
        rx.foreach(tokens.to(list[dict]), _render_highlight_token),
        size="2",
        color_scheme=None if color else "gray",
        color=color,
...
def _business_summary_section() -> rx.Component:
    # A heading/icon row (matching _about_section's style) now sits above
    # the text container, filled with an AI-generated (not CMC/CoinGecko-
    # sourced) plain-language explainer of what the coin does, its business
    # model, and how it makes money, aimed at someone new to crypto (see
    ...
    # distinct topic. The bottom credit row is space-between: "Generated by
    # AI" on the left, the actual model that produced this text (brand logo
    # + short name, e.g. "NVIDIA · Nemotron 3 Ultra") on the right —
    # CoinState._format_model_badge reads this straight from the API
    # response's own "model" field, not just whatever settings.
    # openrouter_model happens to be configured to right now, so it stays
    # accurate even after that default changes. Visually distinct from
    # _about_section's real CMC/CoinGecko-sourced text right above it.
    coin = CoinState.selected_coin
    ...
    return rx.cond(
        ...
        rx.vstack(
            ...
            rx.box(
                ...
                background="var(--gray-a2)",
                width="100%",
                # No glow — a plain solid royalblue border marks this card as AI-generated.
